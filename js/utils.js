...
function inChallenge(layer, id) {
	...
	if (layers[layer].challenges[challenge].countsAs) return tmp[layer].challenges[challenge].countsAs.includes(id)
	return false
}
...
function layOver(obj1, obj2) {
	for (const x in obj2) {
		...
		else if (typeof obj2[x] === "object") layOver(obj1[x], obj2[x]);
		else obj1[x] = obj2[x];
	}
}
...
function toValue(value, oldValue) {
	if (oldValue instanceof Decimal) {
		...
		if (checkDecimalNaN(value)) return Decimal.dZero
		return value
	}
...
function doPopup(type = "none", text = "This is a test popup.", title = "", timer = 3, color = "") {
	switch (type) {
		case "achievement":
			...
			type = "achievement-popup"
			break;
		case "challenge":
			title = "Challenge Complete";
			type = "challenge-popup"
			break;
		default:
...
function adjustPopupTime(diff) {
	for (const popup of activePopups) {
		popup.time -= diff;
		if (popup.time < 0) activePopups.splice(activePopups.indexOf(popup), 1); // Remove popup when time hits 0
	}
}
